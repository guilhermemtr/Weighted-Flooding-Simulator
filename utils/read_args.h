...
#define DET_FH_LIGHT_DIST                                                      \
  (1 << 1)    // Deterministic Few-Heavy   Light  Sender Distribution
#define DET_FH_HEAVY_DIST                                                      \
  (1 << 2)    // Deterministic Few-Heavy   Heavy  Sender Distribution
#define DET_EXP_LIGHT_DIST                                                     \
  (1 << 3)    // Deterministic Exponential Light  Sender Distribution
#define DET_EXP_MEDN_DIST                                                      \
  (1 << 4)    // Deterministic Exponential Median Sender Distribution
#define DET_EXP_HEAVY_DIST                                                     \
  (1 << 5)    // Deterministic Exponential Heavy  Sender Distribution
#define RAND_EXP_RAND_SND_DIST                                                 \
  (1 << 6)    // Random Exponential        Random Sender Distribution
#define RAND_UNIF_RAND_SND_DIST                                                \
  (1 << 7)    // Random Uniform            Random Sender Distribution
#define RAND_GEOM_RAND_SND_DIST                                                \
  (1 << 8)    // Random Geometric          Random Sender Distribution
...
#define CORR_HEAVY_FIRST 2    // Corrupt heavy parties first
#define CORR_LIGHT_FIRST 3    // Corrupt light parties first
...
typedef struct
{
  ...
  /**
   * The parameters below are only relevant for the generation of the stake
   * distributions. The simulator is run with (2^i) * min_nr_parties, for i from
   * 0 up to the largest value for which (2^i) * min_nr_parties <=
   * max_nr_parties
   */
  party_t min_nr_parties;    //!< minimum number of parties
  party_t max_nr_parties;    //!< maximum number of parties

  double ratio_heavy_light;    //!< ratio between heavyest party(ies) and
                               //!< lightest party(ies).
  party_t nr_heavy_parties;    //!< number of heavy parties (only relevant for
                               //!< the few heavy stake distribution

  unsigned int to_show;    //!< the flags that indicate what information to
                           //!< display when running the simulator
 ...
 * @param the input arguments (the first argument is the name of the
 * executable).
